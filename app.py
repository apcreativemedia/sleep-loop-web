...
def fetch_google_trends(keyword_seeds):
    try:
            ...
            try:
                ...
                if q is not None:
                    for _, row in q.head(5).iterrows():
                        ...
                        results.append({"topic": topic, "url": url, "source": "Google Trends US"})
                rising = related.get(seed, {}).get("rising")
                if rising is not None:
                    for _, row in rising.head(3).iterrows():
                        topic = str(row["query"])
                        url = f"https://trends.google.com/trends/explore?q={topic.replace(' ', '%20')}&geo=US"
                        results.append({"topic": f"🔥 {topic}", "url": url, "source": "Google Trends US (rising)"})
                time.sleep(1)
            except Exception as e:
                print(f"[trends] seed '{seed}' failed: {e}", flush=True)
                continue
        seen = set()
        unique = []
        for r in results:
        ...
        print(f"[trends] pytrends failed: {e}", flush=True)
        return []
